    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card title="Profile" className="lg:col-span-1">
          <dl className="space-y-3 text-sm">
            <div className="flex justify-between"><dt className="text-slate-500">Assessment</dt><dd><StatusPill value={child.assessmentStatus} /></dd></div>
            <div className="flex justify-between"><dt className="text-slate-500">Parent</dt><dd className="text-slate-800">{mockEngine.getUserName(child.parentUserId)}</dd></div>
            <div className="flex justify-between"><dt className="text-slate-500">Therapist</dt><dd className="text-slate-800">{mockEngine.getUserName(child.therapistUserId)}</dd></div>
          </dl>
        </Card>

        <Card title="Target Phonemes" className="lg:col-span-2">
          {child.targets.length === 0 ? (
            <p className="text-sm text-slate-400">No targets yet — pending diagnostic screen.</p>
          ) : (
            <ul className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              {child.targets.map((t) => (
                <li key={t.phoneme} className="flex items-center justify-between rounded-lg border border-slate-100 px-4 py-2">
                  <span className="text-sm font-semibold text-slate-800">{t.phoneme}</span>
      ...
      >
            ...
            return (
              <li key={a.gameId} className="flex items-center justify-between py-3">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-brand-50 text-brand-700">{game?.name.slice(0, 1)}</div>
                  <div>
        ...
        <div className="mt-4 border-t border-slate-100 pt-4">
          <p className="mb-2 text-sm font-medium text-slate-700">Manually assign a game</p>
