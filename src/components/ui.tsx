...
export function Card({ title, subtitle, action, children, className }: {
  ...
  export function KpiCard({ label, value, icon, hint, tone = "slate", delta }: {
    label: string;
    value: string | number;
    icon?: ReactNode;
    hint?: string;
...
export function Button({
  ...
  const variants: Record<string, string> = {
    ...
    danger: "bg-red-50 text-red-700 hover:bg-red-100 border border-red-200 dark:bg-red-500/10 dark:text-red-400 dark:hover:bg-red-500/20 dark:border-red-500/30",
  };
...
export function Input({ label, ...props }: { label: string } & React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <label className="block">
      ...
      <input
        {...props}
        className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-800 outline-none focus:border-brand-600 focus:ring-2 focus:ring-brand-100 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:focus:ring-brand-900"
      />
    </label>
  );
}
